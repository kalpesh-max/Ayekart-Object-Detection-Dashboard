...
const CameraGrid = () => {
  ...
  const findLatestDetection = (cameraId) => {
    if (!Array.isArray(detections)) return null;
    return detections
      .filter((d) => d.camera_id === cameraId)
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {cameras.map((camera) => {
          const latest = findLatestDetection(camera.id);
          return (
            <div
              key={camera.id}
            ...
            >
              ...
              <div className="bg-gray-300 dark:bg-slate-600 rounded-lg h-32 mb-3 flex items-center justify-center relative overflow-hidden">
                ...
                {latest && (
                  <div className="absolute bottom-2 left-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded flex items-center justify-between">
                    <span>
                      {latest.object_type} • {Math.round((latest.confidence || 0) * 100)}%
                    </span>
                    <span className="opacity-80">{new Date(latest.timestamp).toLocaleTimeString()}</span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                {latest && (
                  <div className="flex justify-between">
                    <span className="text-gray-500 dark:text-gray-400">Latest Detection:</span>
